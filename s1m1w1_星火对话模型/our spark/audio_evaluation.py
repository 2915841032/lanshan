...
class Ws_Param(object):
    ...
    def __init__(self, APPID, APIKey, APISecret, AudioFile, Text):
        ...
        self.BusinessArgs = {"category": CATEGORY, "sub": SUB, "ent": ENT, "cmd": "ssb", "auf": "audio/L16;rate=16000",
                             "aue": "raw", "text": self.Text, "ttp_skip": True, "aus": 1, "rst": "plain"}
    ...
    try:
        ...
        if code != 0:
            ...
            if (status == 2):
                ...
                total_score_element = root.find('total_score')
                if total_score_element is not None:
                    total_score = total_score_element.get('value')
                    # print(f"total_score is: {total_score}")
                else:
                    print("error: total_score not found")
...
def audio_evaluation(FILE, TEXT):
    #APPID、APISecret、APIKey信息在控制台——语音评测了（流式版）——服务接口认证信息处即可获取
    global wsParam
    global total_score

    total_score = 0
    wsParam = Ws_Param(APPID='e5863ba7',
                       APISecret='NzFjZjA2ODVmMDk5YzllYjZkMDU1YjU2',
                       APIKey='b9e373e0a446ce8f9dbcd8a2a5d2c3f5',
    ...
    return float(total_score)*20

if __name__ == "__main__":
    FILE = 'picture_talk_en.pcm'
    # 待评测文本 utf8 编码，需要加utf8bom 头
    TEXT = '\uFEFF' + "Last weekend, Mary went to the park for a picnic with her friend. They brought a lot of food and enjoyed it very much. Lily went to fly a kite, but she left many rubbish on the ground. Mary cleaned it and put it into the rubbish. Can. The teacher saw it? And she said to marry you are a good girl! What a good girl!"
    # 直接从文件读取的方式
    # TEXT = '\uFEFF'+ open("cn/read_sentence_cn.txt","r",encoding='utf-8').read()

    score = audio_evaluation(FILE, TEXT)
    print(score)
